...
var ribbon = {
	debugMode: false,        //A.K.A. Party Mode
	haltAnimation: false,
	
	// ======== NEW BONES ========
	                         //Note: the angles are backwards from a normal cartisian grid because (0,0) is the top left, not bottom left.
	minAngle: -2,            //expressed in radians (used for INITIAL placement of new bones, line goes up) Note: 1.5708 ~ 90°
	maxAngle: 2,             //    "     "    "     (used for INITIAL placement of new bones, line goes down)
	newBoneShift: 0.0,       //Changes the odds how likely new bones will point in the opposite direction as the previous bone. Higher 
	                         //number means more likely to be different.
	//======== SIZE ========
	boneWidth1200: 180,      //This is the width of each bone when the canvas size is 1200 pixels.
	boneWidth480: 50,        //This is the width of each bone with the canvas size is 480 pixels.
	ribbonWidth: 0,          //This computed [by getRibbonWidth()] value is a actual width of the ribbon segments.
	
	minLength: 300,          //This works similarly to the widths.
	maxLength: 650,
	minActualLength: 0,
	maxActualLength: 0,
	scalingLength: 1000,
	...
	//======== MOVEMENT ========
	velocity: 0.6,           //How fast the ribbon moves across the screen.
	angleVelocity: 0.0002,   //How fast the angles change
	...
	// ======== CANVAS SIZE AND EDGES ========
	ribbon.leftEdge = 0;     //These three edges are recalculated by resizeCanvas().
	ribbon.rightEdge = 0;
	ribbon.topEdge = 0;
	ribbon.bottomBleed = 20; //How far outside canvas points are allowed outside bottom edge.
...
ribbon.getRibbonWidth = function() {
	var viewportWidth = Math.max(480, canvas.width);
	var viewportHeight = Math.max(480, canvas.height);
	if (canvas.width / canvas.height > 1) {
		//When landscape mode use the smallest of width and height. However, if the aspect ratio goes above 2.7, then limit the 
		ribbon.ribbonWidth = ((ribbon.boneWidth1200 - ribbon.boneWidth480)/(1200 - 480)) * (Math.min(viewportWidth, viewportHeight) - 480) + ribbon.boneWidth480;
	} else {
		//otherwise when in portrait mode use the Max
		ribbon.ribbonWidth = ((ribbon.boneWidth1200 - ribbon.boneWidth480)/(1200 - 480)) * (Math.max(viewportWidth, viewportHeight) - 480) + ribbon.boneWidth480;
	}
}
...
ribbon.getOutlines = function() {
	...
	var rays = new Array(); //Holds the imaginary parrallel lines on each side of the bone. Used to calculate intersections.
	for (var i = 0, j = bone.length; i < j; i++) {
		...
		if (i === 0) { //first bone
			...
			bone[i].startOutline.push(findEndPoint(bone[i].startPoint, bone[i].angle.radians + Math.PI / 2, ribbon.ribbonWidth / 2));
			bone[i].startOutline.push(findEndPoint(bone[i].startPoint, bone[i].angle.radians - Math.PI / 2, ribbon.ribbonWidth / 2));
			if (secondBoneGoes === 'down') {
				rays.push({ //rays for bone[0]
					underside: {
						startPoint: bone[i].startOutline[0],
						endPoint: findEndPoint(bone[i].endPoint, bone[i].angle.radians + Math.PI / 2, ribbon.ribbonWidth / 2)},
					topside: {
		...
		if (i === (j - 1)) { //last bone
			bone[i].endOutline.push(findEndPoint(bone[i].endPoint, bone[i].angle.radians - Math.PI / 2, ribbon.ribbonWidth / 2));
			bone[i].endOutline.push(findEndPoint(bone[i].endPoint, bone[i].angle.radians + Math.PI / 2, ribbon.ribbonWidth / 2));
		} else {
			...
			if (secondBoneGoes === 'down') {
				rays.push({ //rays for the next (i + 1) bone.
					underside: {
						startPoint: findEndPoint(bone[i+1].startPoint, bone[i+1].angle.radians + Math.PI / 2, ribbon.ribbonWidth / 2),
						endPoint: findEndPoint(bone[i+1].endPoint, bone[i+1].angle.radians + Math.PI / 2, ribbon.ribbonWidth / 2)},
					topside: {
						startPoint: findEndPoint(bone[i+1].startPoint, bone[i+1].angle.radians - Math.PI / 2, ribbon.ribbonWidth / 2),
						endPoint: findEndPoint(bone[i+1].endPoint, bone[i+1].angle.radians - Math.PI / 2, ribbon.ribbonWidth / 2)}
				});
...
function Bone(previousBone) {
	...
	*/
	...
	do { //Loop until random values for the angle and radius yield a valid bone.
		...
		this.radius = getRandomInt(ribbon.minActualLength, ribbon.maxActualLength);
		...
		escapeLoop++;
	} while ((!(this.positionIs() === 'inside' && this.withinSwingAngle(computedAngle)) && escapeLoop < 20) ||
		(escapeLoop > 19 && !this.withinSwingAngle(computedAngle)) && escapeLoop < 30);
	if (escapeLoop >= 30) {
		console.log('position is ' + this.positionIs() + '. LeftAngle is ' + computedAngle);
		ribbon.haltAnimation = true;
	}
	this.moveCount = 1;
	this.current = {
		angle: this.angle,
...
Bone.prototype.draw = function() {
	...
	if (ribbon.lineCap === 'fold') {
		...
		*/
		...
		ctx.fill();
	} else { //Use standard Canvas line (stroke) styling.
		ctx.lineCap = ribbon.lineCap;
		ctx.lineWidth = ribbon.ribbonWidth;
		if (ribbon.debugMode) {
			if (this.angle.velocity < 0) {
				if (this.topSide) {
					ctx.strokeStyle = 'hsla(' + ribbon.color + ',0.4)';
				} else {
					ctx.strokeStyle = 'hsla(' + ribbon.backColor + ',0.4)';
				}
			} else {
				if (this.topSide) {
					ctx.strokeStyle = 'hsla(' + ribbon.debugColor + ',0.4)';
				} else {
					ctx.strokeStyle = 'hsla(' + ribbon.debugBackColor + ',0.4)';
				}
			}
		}
		ctx.beginPath();
...
Bone.prototype.move = function(previousBone, directionRequest) {
	...
	*/
	...
	//Check canvas limits
	var bonePosition = this.positionIs();
...
function animate() {
	...
	*/
	...
	//over and over again.
	if (!ribbon.debugMode || !ribbon.haltAnimation) {
...
function resizeCanvas() {
	...
	canvas.height = Math.max(getViewport().height - 306, 400); // The subtrahend on this line should be the sum of header height, footer height, and nav bottom margin (30px), footer top padding (10), and 5 more mystery pixels. (for now it is 151 + 110 + 30 + 10 + 5 = 306.
	
	//Ribbon width is based on canvas width, but if screen is in portrait mode, then use canvas height.
	ribbon.getRibbonWidth();
	
	//Edges are based on ribbon.ribbonWidth
	ribbon.topEdge = ribbon.ribbonWidth / 2; //Larger numbers push edge further down into the canvas
	if (ribbon.debugMode) {
		ribbon.leftEdge = canvas.width * 0.2;
	} else {
		ribbon.leftEdge = ribbon.ribbonWidth * -1;
	}
	ribbon.rightEdge = canvas.width - ribbon.leftEdge;

	//Length of bones is dependent upon width between edges.
	ribbon.minActualLength = Math.min(1200, ribbon.rightEdge - ribbon.leftEdge) * (ribbon.minLength / 1200);
	ribbon.maxActualLength = Math.min(1200, ribbon.rightEdge - ribbon.leftEdge) * (ribbon.maxLength / 1200);
...
//If ribbon has been clicked, turn on debug (party) mode.
document.addEventListener('click', function(evt) {
	//Get mouse position relative to the Canvas.
	var canvasLocation = canvas.getBoundingClientRect();
	
    var x = evt.pageX - canvasLocation.left;
    var y = evt.pageY - canvasLocation.top;
	var pixel = ctx.getImageData(x, y, 1, 1).data;
	//pixel is an array containing 4 elements (red, green, blue, and alpha with values from 0 to 255).
	if (pixel[2] > 0) {
		//user double-clicked on something blue-ish so toggle party mode.
		ribbon.debugMode = !ribbon.debugMode;
	}
});
